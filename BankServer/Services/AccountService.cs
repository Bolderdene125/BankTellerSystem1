...
/// Нэгэн зэрэг ирсэн хоёр гүйлгээ давхар хасалт хийхгүйн тулд
/// данс тус бүрт тусдаа lock хадгална.
/// </summary>
public class AccountService
{
    ...
    /// <summary>Дансны lock авах — байхгүй бол шинэ үүсгэнэ.</summary>
    ...
    /// дансны дугаарын string эрэмбээр авна — хоёр тал
    /// үргэлж ижил дарааллаар авдаг тул хэзээ ч бие биенээ хүлээхгүй.
    /// </summary>
    /// <param name="fromAccount">Мөнгө гарах данс.</param>
    /// <param name="toAccount">Мөнгө орох данс.</param>
    /// <param name="amount">Шилжүүлэх дүн (₮).</param>
    /// <returns>Success=true бол амжилттай, false бол шалтгаантай мессеж.</returns>
    public async Task<(bool Success, string Message)> TransferAsync(
        string fromAccount, string toAccount, decimal amount)
    {
        // Deadlock-оос сэргийлэх: lock-уудыг string эрэмбээр авна
